...
app.get("/api/vendors", async (req, res) => {
  try {
    ...
    const vendorData = vendors.map((v) => ({
      ...
      shopImage:
        v.shopImage ||
        `https://picsum.photos/seed/laundry-${v.phoneNumber}/800/600`,
    }));
    res.json(vendorData);
  } catch (err: any) {
    ...
    const cleanupDatabase = async () => {
      try {
        ...
        if (collections.some((c) => c.name === "orders")) {
          ...
          const orderCleanupResult = await mongoose
            .model("Order")
            .deleteMany({ id: null });
          if (orderCleanupResult.deletedCount > 0) {
            console.log(
              `[Database] Cleaned up ${orderCleanupResult.deletedCount} orders with null IDs`,
            );
          }
        }
        ...
        if (collections.some((c) => c.name === "transactions")) {
          ...
          const transCleanupResult = await TransactionModel.deleteMany({
            id: null,
          });
          if (transCleanupResult.deletedCount > 0) {
            console.log(
              `[Database] Cleaned up ${transCleanupResult.deletedCount} transactions with null IDs`,
            );
          }
        }
      } catch (e: any) {
    ...
    const checkOrderTimeouts = async () => {
      try {
        ...
        if (pendingOrders.length > 0) {
          console.log(
            `[Auto-Timeout] Found ${pendingOrders.length} stale orders for refund.`,
          );
          ...
          for (const order of pendingOrders) {
            ...
            try {
              const freshOrder = await mongoose
                .model("Order")
                .findById(order._id)
                .session(session);
              if (
                !freshOrder ||
                !["confirm", "rider_assign_pickup"].includes(freshOrder.status)
              ) {
              ...
              if (freshOrder.paymentMethod === "wallet") {
                const user = await mongoose
                  .model("User")
                  .findOne({ uid: freshOrder.customerUid })
                  .session(session);
                if (user) {
                  user.walletBalance =
                    (user.walletBalance || 0) + freshOrder.totalPrice;
              ...
              console.log(
                `[Auto-Timeout] Order ${freshOrder.id} auto-refunded.`,
              );
            } catch (err) {
              await session.abortTransaction();
              console.error(
                `[Auto-Timeout] Error processing ${order.id}:`,
                err,
              );
            } finally {
              session.endSession();
