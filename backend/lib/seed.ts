...
export const seedAdmin = async () => {
  try {
    const adminEmail = (
      process.env.ADMIN_EMAIL || "ogunwedebo21@gmail.com"
    ).toLowerCase().trim();
    ...
    if (conflictUser) {
      console.log(
        `⚠️  Conflict detected: Email ${adminEmail} or Phone ${adminPhone} is already registered to a non-admin account.`,
      );
      console.log(
        "   Please use a different email/phone in your .env or manually update the user role in MongoDB.",
      );
      return;
    }
