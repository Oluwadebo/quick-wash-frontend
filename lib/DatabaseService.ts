...
export interface UserData {
  ...
  shopAddress?: string;
  landmark?: string;
  consecutiveReturns?: number; // Add this
  // restrictionExpires?: string | null; // Add this
}
...
class DatabaseService {
  ...
  async updateUser(uid: string, data: Partial<UserData>): Promise<UserData> {
    ...
    if (user.trustPoints < 10) {
      ...
      if (user.status === "restricted" && user.restrictionExpires) {
        if (
          new Date().getTime() > new Date(user.restrictionExpires).getTime()
        ) {
          user.status = "active";
  ...
  async claimOrder(
    ...
    if (order.isLocked && order.lockExpires && order.lockExpires > now)
      return false; // Currently being processed by someone else
    ...
    if (status === "picked_up") {
      ...
      if (order.riderUid) {
        ...
        if (rider) {
          ...
          await this.updateUser(rider.uid, {
            walletBalance: (rider.walletBalance || 0) + firstHalf,
          });
    ...
    if (status === "delivered") {
      ...
      if (order.riderUid) {
        ...
        if (rider) {
          ...
          await this.updateUser(rider.uid, {
            walletBalance: (rider.walletBalance || 0) + secondHalf,
          });
          await this.recordTransaction(rider.uid, {
            type: "deposit",
            amount: secondHalf,
  ...
  async runAutoCancel(): Promise<void> {
    ...
    const updatedOrders = await Promise.all(
      orders.map(async (o) => {
        ...
        const isUnassigned =
          (o.status === "rider_assign_pickup" ||
            o.status === "rider_assign_delivery") &&
          !o.riderUid;
          ...
          if (now - orderTime > twentyMins && o.status !== "cancelled") {
            ...
            if (customer) {
              await this.updateUser(o.customerUid, {
                walletBalance:
                  (customer.walletBalance || 0) + (o.totalPrice || 0),
              });
  ...
  async recordTransaction(uid: string, transaction: any) {
    const history = JSON.parse(
      localStorage.getItem(`qw_wallet_history_${uid}`) || "[]",
    );
    history.unshift({
      ...transaction,
  ...
  async getVendorPriceList(vendorUid: string): Promise<any[]> {
    ...
    const allLists = JSON.parse(
      localStorage.getItem("qw_vendor_price_lists") || "{}",
    );
    // Case-insensitive lookup
    const key = Object.keys(allLists).find(
      (k) => k.toLowerCase() === vendorUid.toLowerCase(),
    );
    return key ? allLists[key] : [];
  }

  async saveVendorPriceList(
    vendorUid: string,
    priceList: any[],
  ): Promise<void> {
    await this.delay();
