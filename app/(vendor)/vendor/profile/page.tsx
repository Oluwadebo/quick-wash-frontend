    ...
    <div className="pb-32">
      ...
      <main className="pt-28 px-6 max-w-2xl mx-auto">
        <div className="bg-surface-container-low rounded-[3rem] p-10 text-center mb-8 border border-primary/5 shadow-sm">
          ...
          <h2 className="text-3xl font-headline font-black text-on-surface">{user?.fullName || 'Vendor Name'}</h2>
          <p className="text-primary font-black text-sm uppercase tracking-widest mt-2">{user?.shopName || 'Laundry Shop'}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-primary/5">
            ...
            <h4 className="text-2xl font-headline font-black text-primary">₦124,500</h4>
          </div>
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-primary/5">
        ...
        <div className="space-y-4">
          {[
            { icon: ReceiptText, label: 'Order History', href: '#' },
            { icon: Wallet, label: 'Payout Settings', href: '#' },
            { icon: Settings, label: 'Shop Settings', href: '#' },
          ].map((item, i) => (
